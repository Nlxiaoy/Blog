...
import (
	...
	"server/internal/repo/persistence/gen/model"
	"server/internal/repo/persistence/gen/query"
...
func (r *fileRepo) List(ctx context.Context, offset, limit int, usage *string, sortBy *string, order *string) ([]*entity.File, int64, error) {
	...
	q := f.WithContext(ctx)
	if usage != nil && *usage != "" {
		q = q.Where(f.FileUsage.Eq(*usage))
	}
...
func (r *fileRepo) ListByResource(ctx context.Context, usage string, resourceID int64) ([]*entity.File, error) {
	f := r.query.File
	rows, err := f.WithContext(ctx).Where(f.FileUsage.Eq(usage), f.ResourceID.Eq(resourceID)).Order(f.CreatedAt.Desc()).Find()
	if err != nil {
		return nil, err
	}
	files := make([]*entity.File, len(rows))
...
func (r *fileRepo) ClearResourceIDByResourceAndUsage(ctx context.Context, resourceID int64, usage string) error {
	f := r.query.File
	_, err := f.WithContext(ctx).Where(f.ResourceID.Eq(resourceID), f.FileUsage.Eq(usage)).UpdateSimple(f.ResourceID.Null())
	return err
}
